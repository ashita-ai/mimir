...
## M2: Adversarial Review Pattern (Prosecutor/Defense/Judge)

M1 uses a single-pass review: one model call per task, direct finding output. This is simple and cheap but relies entirely on the model's self-calibration for quality.

M2 introduces a three-phase adversarial pattern to improve finding precision:

1. **Prosecutor** — The initial review call (identical to M1). Produces candidate findings.
2. **Defense** — A second model call that receives the candidate findings and the same slice, tasked with arguing why each finding is a false positive, irrelevant, or low-impact. Returns a rebuttal per finding.
3. **Judge** — A third model call that receives the original finding, the defense rebuttal, and the slice. Renders a verdict: sustain (keep finding), dismiss (suppress), or reduce (lower confidence/severity).

This pattern trades 3x model cost for substantially higher precision. It is gated behind a configuration flag (`review_mode: adversarial`) and only applied to high-confidence findings from the prosecutor phase. Low-confidence findings are not worth the additional model calls.

The prompt templates for defense and judge phases are not specified here — they will be designed during M2 implementation with eval data from M1 to guide calibration.
